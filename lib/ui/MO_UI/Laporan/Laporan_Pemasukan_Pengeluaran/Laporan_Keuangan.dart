...
class _LaporanKeuanganScreenState extends State<LaporanKeuanganScreen> {
        ...
        body: Stack(
          children: [
            Align(
             ...
             Padding(
                padding: EdgeInsets.symmetric(vertical: 100.0, horizontal: 10.0), // Mengurangi padding di sekitar tabel
  ...
  double totalPemasukan = (laporan.totalPenjualan ?? 0).toDouble() + (laporan.totalTip ?? 0).toDouble();
  double totalPengeluaran = laporan.pengeluaranLain.fold(0.0, (sum, item) => sum + (item.harga ?? 0).toDouble()) + (laporan.totalPengadaanBahanBaku ?? 0).toDouble() + (laporan.totalGajiKaryawan ?? 0).toDouble() + (laporan.totalPembayaranPenitip ?? 0.0);
    ...
    return [
      TableRow(
        ...
        children: [
          ...
          Padding(
            padding: const EdgeInsets.all(8.0),
            child: Text('Pemasukan', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.white)),
          ),
          Padding(
            padding: const EdgeInsets.all(8.0),
            child: Text('Pengeluaran', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.white)),
          ),
        ],
      ),
      TableRow(
      ...
      TableRow(
       ...
       TableRow(
        children: [
          ...
          Padding(
            padding: const EdgeInsets.all(8.0),
            child: Text('${formatPengeluaran(laporan.totalPengadaanBahanBaku ?? 0)}'),
          ),
        ],
      ),
      TableRow(
      ...
      TableRow(
        children: [
          ...
          Padding(
            padding: const EdgeInsets.all(8.0),
            child: Text('${formatPengeluaran(laporan.totalPembayaranPenitip ?? 0)}'),
          ),
        ],
      ),
      TableRow(
        ...
        children: [
          ...
          Padding(
            padding: const EdgeInsets.all(8.0),
            child: Text('$formattedTotalPemasukan', style: TextStyle(fontWeight: FontWeight.bold)),
          ),
          Padding(
            padding: const EdgeInsets.all(8.0),
            child: Text('$formattedTotalPengeluaran', style: TextStyle(fontWeight: FontWeight.bold)),
          ),
        ],
      ),
    ];
  ...
  final laporan = await laporanProvider.getLaporanKeuangan(_selectedDate.toString());
  ...
  final tanggalCetak = DateFormat('dd MMMM yyyy', 'id_ID').format(DateTime.now());
    ...
    pw.Page(
      build: (pw.Context context) {
        return pw.Column(
          ...
          children: [
            pw.Text('Laporan Pemasukan dan Pengeluaran', style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),
            ...
            pw.Table.fromTextArray(
              ...
              data: <List<String>>[
                ...
                ['Total Penjualan', '${formatPemasukan(laporan.totalPenjualan ?? 0)}', '-'],
                ['Total Tip', '${formatPemasukan(laporan.totalTip ?? 0)}', '-'],
                ...laporan.pengeluaranLain.map(
                  (e) => [e.namaPengeluaran, '-', '${formatPengeluaran(e.harga ?? 0)}'],
                ),
                ['Total Pengadaan Bahan Baku', '-', '${formatPemasukan(laporan.totalPengadaanBahanBaku) ?? 0}'],
                ['Total Pembayaran Penitip', '-', '${formatPemasukan(laporan.totalPembayaranPenitip) ?? 0.0}'],
                ['Total Gaji Karyawan', '-', '${formatPemasukan(laporan.totalGajiKaryawan) ?? 0}'],
                ['Total', '$formattedTotalPemasukan', '$formattedTotalPengeluaran'],
              ],
            ),
          ],
        );
